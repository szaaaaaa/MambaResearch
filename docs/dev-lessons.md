...
## 阶段四：从端到端流水线到科研超级助手 (2026-04-06)

### 反思 1：端到端全自动研究流程的局限性

**触发**：调研了 AI Scientist、Google Co-Scientist、Karpathy autoresearch 等项目后的系统性反思。

**核心判断**：AI 不太能产生好的科研品味或灵光一现的 idea，只会输出平均结果。端到端全自动完成整个研究流程意义不大。

**证据**：
- Science/AAAS 分析 41M 论文：AI 用户发表量涨 3x，但整体研究主题多样性在收缩——AI 把所有人推向"安全的平均方向"
- Sakana AI-Scientist-v2 首次通过同行评审，但得分在 ICLR workshop 的 top 45%——刚好是"中等偏上"，印证了"平均结果"
- OpenAI 自己的时间表：2028 年才能做到 "legitimate AI researcher"

**结论**：项目方向从"端到端自动研究"转向"人类参与的科研超级助手"。AI 的价值不在于替代科研品味，而在于放大已有品味的执行力。

---

### 反思 2：Planner 硬编码规则把系统锁死在单一流程

**现象**：Planner prompt 中 Rules 11-14 强制了实验触发、实验迭代、论文修订、实验收尾四条固定流程。`_validate_post_report_progression` 在代码层面禁止 review 后使用其他角色。`_fallback_plan` 是 460 行的确定性状态机，写死了 9 步研究流程。`meta_skills.py` 只认 ResearchReport/ReviewVerdict 作为终止信号。

**问题**：用户说"帮我搜几篇论文"，系统会继续往下规划直到产出完整报告。用户说"润色论文"，fallback 可能从 conductor 开始走完整研究流程。Planner 从"灵活决策者"变成了"流程执行器"。

**教训**：
- **Prompt 中的"MUST"规则越多，LLM 的灵活性越低**——硬编码流程规则应改为建议，让 LLM 根据用户意图自主判断
- **终止信号不应硬编码为特定 artifact 类型**——LLM 应该有权判断"用户的需求已经满足了"
- **Fallback 计划不应假设唯一的工作流**——通用 fallback 应该只做"重试当前步骤"或"问用户怎么办"

---

### 反思 3：角色层是多余的抽象

**分析**：当前架构 = 用户请求 → Planner 选角色 → 角色限制技能 → 执行技能。角色层做的事情：
1. 限制可用技能（allowlist）→ skill.yaml 的 allowed_tools 已经定义了
2. 限制输入输出类型 → skill.yaml 的 required/outputs 已经定义了
3. 行为约束（forbidden）→ skill.md 的提示词可以定义
4. "审稿人视角"等角色感 → skill 运行时的 system prompt 可以实现

角色层不增加能力，只增加限制和复杂度。

**关于上下文隔离的讨论**：角色看似能做上下文隔离（不同角色看不同 context），但实际实现中 Planner 每次规划都接收所有 artifacts 摘要，SkillContext 只接收 Planner 指定的 input artifacts——上下文隔离的粒度是技能级别的，不是角色级别的。去掉角色不会让上下文问题变差。

**结论**：去掉角色层，架构简化为：用户请求 → Planner 理解意图 → 直接选技能 → 执行 → 返回结果。Planner 从"研究流程规划器"变成"意图理解 + 技能调度器"。
...
| 时间 | 触发 | 决策 |
|------|------|------|
| 2026-04-06 | 反思端到端流程局限性 | 方向转型：从自动研究流水线 → 科研超级助手 |
| 2026-04-06 | Planner 硬编码规则排查 | 计划移除 Rules 11-14 强制流程、重写 fallback、灵活化终止信号 |
| 2026-04-06 | 角色层价值分析 | 计划去掉角色层，Planner 直接调度技能 |
