...
- ``WorkbenchTab`` 新加可选 ``conversation?: ConversationSummary`` prop；prop 存在时 ``currentBackend`` 跟 ``conversation.backend`` 走，conversation id 用 prop 值跳过 ``ensureConversation`` POST，新 effect 拉 segments 取最近 ``cli_session_id`` 作 Claude/Codex PTY ``resumeId``，并 hydrate messages 表
...
- 验证：tsc clean、build 3.69s、playwright drafts→继续对话 → asset tab 右栏渲染 WorkbenchTab header/PTY 区（替换原 placeholder 文字）；双栏布局静态验证（``isDraft`` 分支共用同一 WorkbenchTab），未做浏览器双栏可视化验证
