...
| 方法 | 路径 | 说明 |
|------|------|------|
| `WS`     | `/api/terminal/{backend}` | PTY 直连 CLI 二进制（`backend=claude` 或 `codex`），text+binary 双向流；query: `cwd` / `provider` / `resume` / `conversation_id` |
...
聊天主路径：浏览器 `<TerminalPane>` (xterm.js) ↔ FastAPI WS ↔ ConPTY (`pywinpty`) ↔ `claude` / `codex` 子进程；HITL y/n 在终端内由 CLI 自己处理，前端不再有 PermissionModal / SSE composer 聊天主流程。`/api/codex/sessions/*` 属于 legacy app-server 兼容层，不再定义 Workbench 交互方向。
