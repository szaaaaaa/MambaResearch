...
13. PAPER REVISION: When you see a ReviewVerdict with verdict=needs_revision (visible in the hint field), plan a single writer node — draft_report with the prior ReviewVerdict as input AND needs_review=true. DO NOT emit a separate reviewer node yourself; the runtime auto-triggers review_artifact whenever needs_review=true (see RULE 17). Repeat until verdict=accept or max_rewrite_cycles is reached (the post-review policy validator forces terminate after the cap).
14. EXPERIMENT COMPLETION: When ExperimentIteration has should_continue=false or strategy=early_stop, transition to the reporting phase: analyze_metrics (or aggregate_results if multiple experiment rounds exist) → generate_figures → draft_report, then set terminate=true. DO NOT auto-append a review_artifact node here. Only add reviewer when explicitly responding to a pre-existing ReviewVerdict that says needs_revision (see RULE 13). Speculative review on a first-pass report just creates a ReviewVerdict that often scores below threshold and forces a constrained replan you cannot easily satisfy.
15. SKILL INTERNAL BUG → SELF-REPAIR: ONLY when the latest observations show status="needs_replan" with error_type = skill_error or llm_error (the skill's OWN code is buggy — exception, contract violation, parse failure), plan a self-repair pair:
    (a) analyst node with allowed_skills=["reflect_on_failure"], inputs=[], expected_outputs=["ReflectionReport"]. Put the failed skill_id in goal as "skill_id: <name> 失败分析" — <name> is in observation.what_was_tried (e.g., "skill:search_papers" → "search_papers").
    (b) experimenter node with allowed_skills=["optimize_skill"], inputs reference (a)'s ReflectionReport via the deterministic future ref, expected_outputs=["SkillPatch"]. The runtime auto-reloads the registry after SkillPatch fires, so the NEXT planning iteration can retry the original chain with the evolved skill.
    DO NOT trigger this path for error_type = workload_error / tool_failure / timeout / policy_block — those are NOT skill source bugs and reflection on the skill source will produce no useful change.
16. WORKLOAD ERROR → EXPERIMENT ITERATION REPAIR: When the latest observations show status="needs_replan" with error_type = workload_error, the skill itself is fine but the LLM-generated code/data it drove (e.g., a generated train.py) failed. The artifact store will contain an ExperimentResults with status="failed" and a mutable_files_snapshot. Plan an analyst node calling optimize_experiment to consume that failed ExperimentResults — it will emit ExperimentIteration with strategy="refine" or "pivot" plus failure lessons. The next planning iteration then re-runs design_experiment (which will see the ExperimentIteration in artifact hints and rewrite the broken files) → run_experiment. DO NOT call reflect_on_failure / optimize_skill here — the skill source is not the bug.
17. AUTO-REVIEW (needs_review FLAG): To request a quality gate on a writer node, set needs_review=true on that node. After the node completes successfully and produces a ResearchReport, the runtime automatically synthesizes a reviewer sub-node and runs review_artifact, producing a ReviewVerdict that the next planning iteration can react to (see RULE 13). Default is false. NEVER emit a planner-level review_artifact / reviewer node yourself — the auto-trigger via needs_review is the only sanctioned path. A failed auto-review is non-fatal and does not change the writer node's status.
"""
