...
import { Archive, Clock, Copy, Cpu, Pencil, Plus, Settings2, Terminal, Trash2 } from 'lucide-react';
...
interface SidebarProps {
  ...
  onOpenSettings: () => void;
  activeTab: 'run' | 'history' | 'skills' | 'workbench';
  onTabChange: (tab: 'run' | 'history' | 'skills' | 'workbench') => void;
}
      ...
      <aside className="flex h-full w-full shrink-0 flex-col bg-[#f3f4f6]">
        <div className="border-b border-slate-200 px-4 py-4">
          ...
          <div className="mt-2 flex">
            <button
              type="button"
              onClick={() => onTabChange(activeTab === 'workbench' ? 'run' : 'workbench')}
              className={`flex flex-1 items-center justify-center gap-2 rounded-2xl px-3 py-2 text-sm font-medium transition ${
                activeTab === 'workbench'
                  ? 'bg-slate-900 text-white'
                  : 'border border-slate-200 bg-white text-slate-700 hover:bg-slate-50'
